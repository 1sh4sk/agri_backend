...
class Service {
    ...
    validateRole(role) {
        ...
        if (!validRoles.includes(role)) {
            throw new customError_1.CustomError(`Invalid role. Must be one of: ${validRoles.join(', ')}`, 400);
        }
    }
    async register(payload) {
        ...
        if (existingByPhone) {
            throw new customError_1.CustomError("User already registered with this phone number.", 409);
        }
        if (email) {
            const existingByEmail = await this.repository.findAuthByEmail(email);
            if (existingByEmail) {
                throw new customError_1.CustomError("User already registered with this email.", 409);
            }
        }
    ...
    async verifyOtp(payload) {
        ...
        if (!tempAuth) {
            throw new customError_1.CustomError("No registration request found for this phone number.", 404);
        }
        if (tempAuth.expiresAt && tempAuth.expiresAt < new Date()) {
            await this.repository.deleteTempAuth(tempAuth.phoneNumber);
            throw new customError_1.CustomError("OTP has expired. Please request a new one.", 400);
        }
    ...
    async sendEmailOtpForVerification(payload, userId) {
        ...
        if (existingEmail && existingEmail.id !== userId) {
            throw new customError_1.CustomError("Email is already registered by another user.", 409);
        }
    ...
    async verifyEmailOtp(payload, userId) {
        ...
        if (!auth.emailOtp) {
            throw new customError_1.CustomError("No email OTP found. Please request a new one.", 400);
        }
    ...
    async login(payload) {
        ...
        if (!auth) {
            throw new customError_1.CustomError("User not found. Please register first.", 404);
        }
    ...
    async verifyLoginOtp(payload) {
        ...
        if (!auth.loginOtp) {
            throw new customError_1.CustomError("No login OTP found. Please request a new one.", 400);
        }
        ...
        await this.repository.clearLoginOtp(auth.id);
        const roleUser = await this.repository.findRoleBasedUser(auth.role, auth.id);
        const tokens = auth_1.JWTUtils.generateTokenPair({
            id: auth.id,
            email: auth.email || '',
            userName: auth.userName,
    ...
    async refreshAccessToken(payload) {
        ...
        if (!auth) {
            throw new customError_1.CustomError("User not found or has been deleted.", 404);
        }
    ...
    async resendRegistrationOtp(phoneNumber) {
        const tempAuth = await this.repository.findTempByPhone(phoneNumber);
        if (!tempAuth) {
            throw new customError_1.CustomError("No registration request found for this phone number. Please register first.", 404);
        }
        const existingUser = await this.repository.findAuthByPhone(phoneNumber);
        if (existingUser) {
            throw new customError_1.CustomError("User already registered. Please login instead.", 409);
        }
        const smsOtp = this.generateOtp();
    ...
    async resendEmailVerificationOtp(userId) {
        ...
        if (!auth.email) {
            throw new customError_1.CustomError("No email found for this user. Please add an email first.", 400);
        }
        if (auth.emailVerified) {
            throw new customError_1.CustomError("Email is already verified.", 400);
        }
